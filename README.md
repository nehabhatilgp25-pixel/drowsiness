...
Click [here](https://drowsiness-jade.vercel.app/) to access to online web dashboard.

## Future Scope

This version of the system is just a prototype. Future versions could include the ESP32-CAM so that the system can physically be implemented in an actual car (right now it operates on the laptop). Future versions could also check the car's location using GPS and accordingly call the driver's family if the driver is asleep for too long.
